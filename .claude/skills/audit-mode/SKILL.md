...
- **Worktree isolation.** Each audit agent should run in its own git worktree so one agent's read state never sees another's drafts. (`git worktree add` per agent; the agent writes to `agent_docs/audits/<DATE>/<slug>.md` directly.) When the cycle closes, run the worktree cleanup steps in `.claude/skills/orchestrate-zebu/SKILL.md` §"Wave closeout" — the same accumulation pattern that motivated the orchestrate-zebu rule applies to audit cycles.
