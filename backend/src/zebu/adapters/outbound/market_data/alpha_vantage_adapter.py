...
class AlphaVantageAdapter:
    ...
    def __init__(
        ...
        Args:
            ...
            skip_market_hours_check: Skip weekend/holiday check (for E2E tests)
        """
    ...
    async def get_current_price(self, ticker: Ticker) -> PricePoint:
        ...
        # Skip this check in E2E test mode to allow tests to run on weekends
        now = datetime.now(UTC)
        if not self.skip_market_hours_check and not MarketCalendar.is_trading_day(
            now.date()
        ):
    ...
    async def get_batch_prices(self, tickers: list[Ticker]) -> dict[Ticker, PricePoint]:
        ...
        2. Checking database for uncached tickers (with weekend awareness)
        3. Only fetching uncached tickers from API (one by one, if trading day)
        4. Returning partial results if some tickers fail
        ...
        # Check if today is a trading day (skip check in E2E test mode)
        now = datetime.now(UTC)
        is_trading_day = (
        ...
        if not is_trading_day:
            logger.info(
                "Markets closed, using last trading day prices for batch request",
                current_date=now.date().isoformat(),
                last_trading_day=(
                    last_trading_day.date().isoformat() if last_trading_day else None
                ),
