...
class InMemoryTransactionRepository:
    ...
    async def save(
        ...
        *,
        ...
        ``api_key_id`` and ``trigger_id`` are accepted for protocol
        compatibility (Phase H2 + Phase F-5) but ignored in this in-memory
        implementation. The activity-feed aggregator and trigger fire log
        target the SQLModel adapter where the columns live.
        """
        del api_key_id  # in-memory adapter doesn't track originating credential
        del trigger_id  # in-memory adapter doesn't track originating trigger fire
        with self._lock:
    ...
    async def save_all(
        ...
        *,
        ...
        ``api_key_id`` and ``trigger_id`` are accepted for protocol
        compatibility but ignored in this in-memory adapter; see
        :meth:`save` for rationale.
        """
        del api_key_id
        del trigger_id
        if not transactions:
