...
main() {
    ...
    log_step "Creating application directory on VM..."
    
    ssh "$VM_DEFAULT_USER@$vm_ip" "mkdir -p $APP_DIR"
    
    # Transfer application code to VM
    log_step "Transferring application code to VM..."
    log_info "This may take a few minutes..."
    
    # Create a tarball of the repository (excluding git, node_modules, etc.)
    local tarball="/tmp/papertrade-deploy-$$.tar.gz"
    ...
    if ! scp "$tarball" "$VM_DEFAULT_USER@$vm_ip:/tmp/papertrade.tar.gz"; then
        rm -f "$tarball"
        error_exit "Failed to transfer application code to VM"
    fi
    
    rm -f "$tarball"
    
    # Extract on VM
    ssh "$VM_DEFAULT_USER@$vm_ip" "cd $APP_DIR && tar xzf /tmp/papertrade.tar.gz && rm /tmp/papertrade.tar.gz"
    
    log_success "Application code transferred"
